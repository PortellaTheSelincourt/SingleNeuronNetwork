    ...
    public void train(float learningRate){
        ...
        while (totalError != 0){
            ...
            for(int i=0;i<output.length;i++){
                ...
                float error = Math.abs(output[i]-calculatedOutput);                 // 'output' = ideal answer

                totalError += error;

                for(int j=0;j<numOfWeights;j++){                                     // error calculator weight(n+1)=weight(n)+input*error*learningRate(<1)
